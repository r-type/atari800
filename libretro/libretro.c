...
void retro_audiocb(signed short int *sound_buffer,int sndbufsize){
   ...
   if(pauseg==0)for(x=0;x<sndbufsize*2;x+=2)audio_cb(sound_buffer[x],sound_buffer[x+1]);	
}
...
int gui_poll_events(void)
{
  ...
  current_tus/=1000;
   ...
   if(current_tus - last_tus >= 1000/60)
   { 
...
extern int amain (int argc, char* argv[]);
/*
#include <fstream>
#include <string>
*/
...
int pre_main(const char *argv)
{
   ...
   // handle cmd file 
   if (strlen(RPATH) >= strlen("cmd")){
	if(!strcasecmp(&RPATH[strlen(RPATH)-strlen("cmd")], "cmd"))
                       i=loadcmdfile(argv);                        
   }
   ...
   Only1Arg = (strcmp(ARGUV[0],"atari800") == 0) ? 0 : 1;
...
//		Add_Option("4");
   ...
   amain(PARAMCOUNT,( char **)xargv_cmd); 
...
void retro_set_environment(retro_environment_t cb)
{
   ...
   struct retro_variable variables[] = {
      {
         "atari800_sdl_analog","Use Analog; OFF|ON",
      },
      { NULL, NULL },
   };
...
void retro_get_system_info(struct retro_system_info *info)
{
	...
	info->library_name = "atari800-sdl";
...
void update_geometry()
{
   ...
   system_av_info.geometry.aspect_ratio = 4.0 / 3.0;//retro_aspect;
   environ_cb(RETRO_ENVIRONMENT_SET_GEOMETRY, &system_av_info);
   LOGI("GEOMETRY: width=%d height=%d\n",system_av_info.geometry.base_width,system_av_info.geometry.base_height);
 
}

void update_timing(int PALMODE)
{
   struct retro_system_av_info system_av_info;
   ...
   system_av_info.timing.fps = (PALMODE ? 49.8607597:59.9227434);
...
void retro_init(void)
{
   ...
   sprintf(retro_system_data, "%s%catari800\0",RETRO_DIR,slash);
...
*/
	...
	if (NEWGAME_FROM_OSD == 2){
	      update_geometry();
	      LOGI("ChangeGEO: w:%d h:%d a:%f\n",retrow,retroh,(float)retrow/(float)retroh);
	      NEWGAME_FROM_OSD=0;
	}
